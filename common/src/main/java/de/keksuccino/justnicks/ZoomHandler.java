...
public class ZoomHandler {
    ...
    public static float zoomModifier = JustNicks.getOptions().baseZoomFactor.getValue();
    ...
    public static boolean isZooming() {
        ...
        if (Minecraft.getInstance().options.getCameraType().isMirrored() && !JustNicks.getOptions().allowZoomInMirroredView.getValue()) {
            return false;
        }
    ...
    public static boolean shouldZoomInOutSmooth() {
        return JustNicks.getOptions().smoothZoomInOut.getValue();
    }

    public static boolean shouldHideArmsWhenZooming() {
        return isZooming() && JustNicks.getOptions().hideArmsWhenZooming.getValue();
    }
    ...
    public static void onMouseScroll(@NotNull MouseScrollFeedback feedback, double deltaX, double deltaY) {
        ...
        if (isZooming()) {
            ...
            if (deltaY < 0) {
                zoomModifier += JustNicks.getOptions().zoomOutPerScroll.getValue();
            } else if (deltaY > 0) {
                zoomModifier -= JustNicks.getOptions().zoomInPerScroll.getValue();
            }
