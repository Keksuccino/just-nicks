...
- Suffix every unique field or helper with `_JustNicks`. Static finals use all caps with `_JUSTNICKS`, and injected method names follow the `before/after/on/wrap/cancel_<VanillaMethod>_JustNicks` pattern. Accessor/invoker methods also end in `_JustNicks`.
...
- JustNicks has access to Mixin Extras.
- Prefer using features from Mixin Extras instead of using normal Mixin redirects or overrides.
- When leveraging Mixin Extras (`WrapOperation`, `WrapWithCondition`, etc.), name helpers after the intent (`wrap_..._JustNicks`, `cancel_..._JustNicks`) and call the provided `Operation` when returning to vanilla flow.
